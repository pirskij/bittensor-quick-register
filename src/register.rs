...
impl QuickRegister {
    ...
    async fn perform_burn_registration(
        ...
        if balance < burn_amount {
            return Err(anyhow!("Insufficient balance. Required: {}, Available: {}", 
                utils::format_tao(burn_amount), 
                utils::format_tao(balance)
            ));
        }
    ...
    async fn verify_registration(&self, netuid: u16, hotkey_account: &AccountId32) -> Result<()> {
        ...
        for attempt in 1..=5 {
            ...
            sleep(Duration::from_secs(12)).await; // Ждем блок
            
            match self.client.check_registration(netuid, hotkey_account).await? {
                Some(neuron) => {
                    println!("✅ Registration verified! Assigned UID: {}", neuron.uid);
                    println!("   Active: {}", if neuron.active { "Yes" } else { "No" });
                    println!("   Stake: {}", utils::format_tao(neuron.stake.iter().map(|(_, s)| s).sum::<u64>()));
                    return Ok(());
                }
                None => {
                    if attempt < 5 {
        ...
        println!("⚠️ Registration may still be processing. Check status manually in a few minutes.");
        Ok(())
    }
    ...
    pub async fn estimate_registration_cost(&self, netuid: u16) -> Result<()> {
        ...
        println!("│  ├─ USD equivalent: ~${:.2} (assuming $200/TAO)", subnet_info.burn as f64 / 1e9 * 200.0);
    ...
    pub async fn show_subnet_info(&self, netuid: u16) -> Result<()> {
        ...
        println!("   Current block: {}", current_block);
        println!("   Burn cost in USD: ~${:.2}", subnet_info.burn as f64 / 1e9 * 200.0);
    ...
    pub async fn monitor_multiple_neurons(&self, registrations: Vec<(u16, String)>) -> Result<()> {
        ...
        for (netuid, hotkey_path) in registrations {
            println!("\n📍 Subnet {} - {}", netuid, utils::format_account_short(
                &key_utils::account_id_from_string(&hotkey_path)?
            ));
    ...
    pub async fn auto_register_with_retry(
        ...
        println!("🔄 Auto registration with retry (max {} attempts)", max_retries);
        
        for attempt in 1..=max_retries {
            println!("\n🚀 Registration attempt {}/{}", attempt, max_retries);
            
            match self.register_to_subnet(
                netuid,
                wallet_path,
                hotkey_path,
                None,
    ...
    pub async fn execute_batch_operations(&self, config_path: &str) -> Result<()> {
        ...
        for (i, operation) in batch_config.operations.iter().enumerate() {
            println!("\n🔄 Operation {}/{}: {}", i + 1, batch_config.operations.len(), operation.operation);
            ...
            match operation.operation.as_str() {
                ...
                "auto_register" => {
                    if let Some(wallet) = &operation.wallet {
                        ...
                        match self.auto_register_with_retry(
                            operation.subnet,
                            wallet,
                            &operation.hotkey,
                            max_retries,
